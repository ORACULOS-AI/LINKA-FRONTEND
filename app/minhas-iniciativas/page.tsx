...
import { Plus, Trash2, Edit, Users, Lightbulb, CheckCircle, Clock, Trophy, Sparkles } from 'lucide-react'
...
import { InitiativeCreationModal } from './components/initiative-creation-modal'
...
export default function InitiativesManagementPage() {
  ...
  const { activeCount, pendingCount, completedCount, totalCount } = useMemo(
    () => ({
      totalCount: initiatives?.length || 0,
      activeCount: initiatives?.filter((i) => i.status === StatusIniciativa.ATIVA).length || 0,
      pendingCount: initiatives?.filter((i) => i.status === StatusIniciativa.PENDENTE).length || 0,
      completedCount: initiatives?.filter((i) => i.status === StatusIniciativa.CONCLUIDA).length || 0,
    }),
    [initiatives],
  )
  ...
  const sortedInitiatives = useMemo(() => {
    ...
    return [...initiatives].sort(
      (a, b) =>
        new Date(b.data_cadastro || 0).getTime() - new Date(a.data_cadastro || 0).getTime(),
    )
  }, [initiatives])
        ...
        >
          ...
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">
              Você precisa ter um negócio cadastrado
            </h2>
            <p className="text-gray-600 max-w-md">
              Cadastre seu primeiro negócio antes de criar iniciativas
            </p>
          </div>
          <Button
      ...
      return (
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600 max-w-md">
              Cadastre sua primeira iniciativa para começar a gerenciar seus projetos
            </p>
          </div>
          <Button
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Tem certeza que deseja excluir "{initiative.titulo}"?
                ...
                {initiative.participantes && initiative.participantes.length > 0 && (
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-700">
                      ...
                      <span>{initiative.participantes.length} participante(s)</span>
                    </div>
                  </div>
                )}
              </CardContent>
          ...
          <div className="relative z-10 container mx-auto px-4 py-16">
            ...
            >
              ...
              >
                ...
                <span className="text-sm font-semibold">Gestão de Iniciativas</span>
              </motion.div>
              ...
              >
                Gerencie suas iniciativas, acompanhe o progresso e mantenha suas informações atualizadas
              </motion.p>
              ...
              >
                {[
                  { icon: Lightbulb, label: 'Total', value: totalCount, color: 'yellow' },
                  { icon: CheckCircle, label: 'Aprovadas', value: activeCount, color: 'green' },
                  { icon: Clock, label: 'Pendentes', value: pendingCount, color: 'amber' },
                  { icon: Trophy, label: 'Concluídas', value: completedCount, color: 'blue' },
                ].map((stat, idx) => (
                  <motion.div
                    key={stat.label}
                    initial={{ opacity: 0, scale: 0.8 }}
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          >
            {error ? (
              ...
              >
                <p className="font-semibold mb-2">Erro ao carregar iniciativas</p>
