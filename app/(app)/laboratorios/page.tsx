...
function LabCard({ lab, idx }: { lab: LabSummary; idx: number }) {
  ...
  return (
    <Link href={`/vitrine/laboratorios/${lab.uid}`} className="vcard">
...
function LabRow({ lab, idx }: { lab: LabSummary; idx: number }) {
  const cor = getCor(idx)
  return (
    <Link href={`/vitrine/laboratorios/${lab.uid}`} className="card is-clickable">
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <div className="empty">
              ...
              <p>Tente ajustar os filtros ou cadastre um novo laboratório.</p>
            ...
            <div className={view === 'grid' ? 'grid-3' : 'col'} style={view === 'grid' ? { gap: 16 } : { gap: 12 }}>
              {sorted.map((l, i) => view === 'grid'
                ? <LabCard key={l.uid} lab={l} idx={i} />
                : <LabRow key={l.uid} lab={l} idx={i} />)}
            </div>
          )}
        </div>
      </div>
