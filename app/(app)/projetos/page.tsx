...
function ProjetoCard({ proj, idx }: { proj: Iniciativa; idx: number }) {
  ...
  return (
    <Link href={`/vitrine/projetos/${proj.uid}`} className="vcard">
...
function ProjetoRow({ proj, idx }: { proj: Iniciativa; idx: number }) {
  const cor = getCor(idx)
  return (
    <Link href={`/vitrine/projetos/${proj.uid}`} className="card is-clickable">
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <div className="empty">
              ...
              <p>Tente ajustar os filtros ou crie um novo projeto.</p>
            ...
            <div className={view === 'grid' ? 'grid-3' : 'col'} style={view === 'grid' ? { gap: 16 } : { gap: 12 }}>
              {sorted.map((p, i) => view === 'grid'
                ? <ProjetoCard key={p.uid} proj={p} idx={i} />
                : <ProjetoRow key={p.uid} proj={p} idx={i} />)}
            </div>
          )}
        </div>
      </div>
